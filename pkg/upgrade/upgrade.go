...
func (u *Upgrader) Run(ctx context.Context) error {
	...
	if method == upgrade.BootMethodOther {
...
func (u *Upgrader) update(ctx context.Context) error {
	...
	env := environment.MustGet()
	gw := stubs.NewRegistrarGatewayStub(u.zcl)
	registrarVersion, testFarms, err := getRolloutConfig(ctx, gw)
	if err != nil {
		return errors.Wrap(err, "failed to get rollout config and version")
	}

	remoteVer := remote.Target[strings.LastIndex(remote.Target, "/")+1:]
	// we don't have is light anymore
	// if kernel.GetParams().IsLight() {
	// 	if env.RunningMode != environment.RunningDev && (remoteVer != registrarVersion.VersionLight) {
	// 		// nothing to do! hub version is not the same as the chain
	// 		return nil
	// 	}
	// } else {
	if env.RunningMode != environment.RunningDev && (remoteVer != registrarVersion.Version) {
		// nothing to do! hub version is not the same as the registrar
		return nil
	}
	// }

	if !registrarVersion.SafeToUpgrade {
