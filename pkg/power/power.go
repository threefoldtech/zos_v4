...
func (p *PowerServer) syncSelf() error {
	power, err := p.registrarGateway.GetPowerTarget(context.Background())
	if err != nil {
		return err
	}
...
func (p *PowerServer) powerUp(node *client.Node, reason string) error {
	...
	if mac == "" {
		return fmt.Errorf("can't find mac address of node '%d'", node.NodeID)
	}
...
func (p *PowerServer) event(event *pkg.PowerTargetChangeEvent) error {
	...
	node, err := p.registrarGateway.GetNode(context.Background(), uint64(event.NodeID))
	if err != nil {
		return err
	}
...
func (p *PowerServer) setNodePowerState(up bool) error {
	...
	*/
	...
	_, err = p.registrarGateway.SetNodePowerState(context.Background(), up)
	return err
}
