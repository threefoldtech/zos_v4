...
func (e *NativeEngine) validate(ctx context.Context, dl *gridtypes.Deployment, noValidation bool) (context.Context, error) {
	if e.registrarGateway == nil {
		return ctx, fmt.Errorf("substrate is not configured in engine")
	}

	contract, subErr := e.registrarGateway.GetContract(ctx, dl.ContractID)
	if subErr.IsError() {
	...
	rent, subErr := e.registrarGateway.GetNodeRentContract(ctx, uint32(e.nodeID))
	...
	if uint32(contract.ContractType.NodeContract.Node) != uint32(e.nodeID) {
